...
const SettingsScreen: React.FC<SettingsScreenProps> = ({ onBack, onDisconnect, onNavigate }) => {
  ...
  const { userProfile, updateUserProfile, appSettings, updateAppSettings, spendingLimit, setSpendingLimit, isLockedDown, setIsLockedDown, twoFactorEnabled, setTwoFactorEnabled } = useWallet();
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center justify-between p-5 bg-white/5 rounded-2xl border border-white/5">
            <div className="flex items-center gap-4">
              ...
              <span className="text-sm font-black text-white uppercase tracking-wider">Biometric Access</span>
            </div>
            <div
              onClick={() => {
                const newValue = !appSettings.biometricsEnabled;
                updateAppSettings({ biometricsEnabled: newValue });
                showToast(newValue ? 'Biometrics Enabled' : 'Biometrics Disabled', 'success');
              }}
              className={`w-12 h-7 rounded-full relative p-1 cursor-pointer transition-colors ${appSettings.biometricsEnabled ? 'bg-primary' : 'bg-white/10'}`}
            >
              <div className={`absolute top-1 bottom-1 aspect-square bg-white rounded-full shadow-lg transition-all ${appSettings.biometricsEnabled ? 'right-1' : 'left-1'}`}></div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between p-5 bg-white/5 rounded-2xl border border-white/5">
            <div className="flex items-center gap-4">
              ...
              <span className="text-sm font-black text-white uppercase tracking-wider">Security Alerts</span>
            </div>
            <div
              onClick={() => {
                const newValue = !appSettings.notificationsEnabled;
                updateAppSettings({ notificationsEnabled: newValue });
                showToast(newValue ? 'Security Alerts Enabled' : 'Security Alerts Disabled', 'success');
              }}
              className={`w-12 h-7 rounded-full relative p-1 cursor-pointer transition-colors ${appSettings.notificationsEnabled ? 'bg-primary' : 'bg-white/10'}`}
            >
              <div className={`absolute top-1 bottom-1 aspect-square bg-white rounded-full shadow-lg transition-all ${appSettings.notificationsEnabled ? 'right-1' : 'left-1'}`}></div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between p-5 bg-white/5 rounded-2xl border border-white/5">
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col">
                <span className="text-sm font-black text-white uppercase tracking-wider">Two-Factor Auth</span>
                <span className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Google Authenticator</span>
              </div>
            </div>
            <div
              onClick={() => {
              ...
              className={`w-12 h-7 rounded-full relative p-1 cursor-pointer transition-colors ${twoFactorEnabled ? 'bg-primary' : 'bg-white/10'}`}
            >
              <div className={`absolute top-1 bottom-1 aspect-square bg-white rounded-full shadow-lg transition-all ${twoFactorEnabled ? 'right-1' : 'left-1'}`}></div>
            </div>
          </div>

          <div className="p-5 bg-white/5 rounded-2xl border border-white/5 flex flex-col gap-4">
            <div className="flex items-center gap-4">
              <span className="material-symbols-outlined text-gray-400">savings</span>
              <span className="text-sm font-black text-white uppercase tracking-wider">Smart Spending Limits</span>
            </div>
            <div className="flex flex-col gap-3 pl-10">
              <div className="flex justify-between items-center">
                <span className="text-xs text-gray-500 font-bold uppercase">Daily Limit</span>
                <span className="text-xs text-white font-black">₦{spendingLimit.daily.toLocaleString()}</span>
              </div>
              ...
              <button
                onClick={() => {
                  const newLimit = prompt("Enter new daily limit:", spendingLimit.daily.toString());
                  if (newLimit && !isNaN(parseFloat(newLimit))) {
                    setSpendingLimit({ ...spendingLimit, daily: parseFloat(newLimit) });
                    showToast("Daily limit updated", "success");
                  }
                }}
                className="text-[10px] text-primary font-black uppercase tracking-widest text-left hover:text-white transition-colors">Adjust Limits</button>
            </div>
          </div>
            ...
            <button
              onClick={() => {
                if (isLockedDown) {
                  ...
                  showToast("Emergency Lockdown Lifted", "success");
                } else {
                  if (confirm("ARE YOU SURE? This will disable all transfers.")) {
                    setIsLockedDown(true);
                    showToast("EMERGENCY LOCKDOWN ACTIVATED", "error");
                  }
                }
              }}
              className={`w-full py-4 ${isLockedDown ? 'bg-green-600 hover:bg-green-500' : 'bg-red-500 hover:bg-red-600'} text-white rounded-xl font-black text-[10px] uppercase tracking-widest active:scale-95 transition-all shadow-lg shadow-red-500/20`}
            >
              {isLockedDown ? 'Lift Lockdown' : 'Activate Lockdown'}
            </button>
          </div>
        </div>
      </BottomSheet>
