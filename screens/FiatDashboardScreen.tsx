...
const FiatDashboardScreen: FC<FiatDashboardScreenProps> = ({ onBack }) => {
    ...
    const [transferRegion, setTransferRegion] = useState<'Africa' | 'Europe'>('Africa');
    ...
    useEffect(() => {
        const fetchBanks = async () => {
            try {
                const b = await afriexClient.getInternationalBanks(transferRegion);
                setBanks(b);
            } catch (error) {
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            ...
            <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 mb-16">
                <div className="flex-1">
                    <span className="text-[11px] font-black text-text-tertiary uppercase tracking-[0.4em] mb-4 flex items-center gap-3">
                        Total Balance
                    ...
                    <div className="flex items-baseline gap-3 mb-2 flex-wrap">
                        <span className="text-4xl sm:text-6xl font-black text-white tracking-tighter font-display">
                            {balanceVisible ? `₦${fiatData?.total_balance.toLocaleString()}` : '••••••••'}
                        </span>
                ...
                <div className="w-full lg:w-[450px] bg-surface-dark/30 rounded-[2.5rem] p-6 sm:p-8 border border-white/5 glass relative overflow-hidden flex flex-col justify-center">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <button onClick={() => setShowReportsModal(true)} className="text-[9px] font-black text-text-tertiary uppercase tracking-widest hover:text-white transition-colors">View Report</button>
                    </div>
            ...
            <div className="flex flex-col lg:flex-row gap-12">
                <div className="flex-1 lg:flex-[2] flex flex-col gap-8">
                    <div className="bg-surface-dark/30 rounded-[2.5rem] p-10 border border-white/5 glass">
                        {/* Region Selector */}
                        <div className="flex bg-white/5 p-1 rounded-2xl mb-10 w-full sm:w-fit self-center">
                            <button
                                onClick={() => setTransferRegion('Africa')}
                                className={`flex items-center gap-3 px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${transferRegion === 'Africa' ? 'bg-primary text-white shadow-lg' : 'text-text-tertiary hover:text-white'}`}
                            >
                                <span className="material-symbols-outlined text-sm">public</span>
                                Africa (Local)
                            </button>
                            <button
                                onClick={() => setTransferRegion('Europe')}
                                className={`flex items-center gap-3 px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${transferRegion === 'Europe' ? 'bg-primary text-white shadow-lg' : 'text-text-tertiary hover:text-white'}`}
                            >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                            ...
                            <div>
                                <label className="text-[10px] font-black text-text-tertiary uppercase tracking-widest mb-3 block">Recipient Bank</label>
                                <div className="relative">
                                    <select className="w-full h-14 bg-white/5 border border-white/10 rounded-xl px-4 focus:outline-none focus:border-primary text-sm font-bold appearance-none pr-10">
                                        {banks.map(bank => (
                                            <option key={bank.id} value={bank.name} className="bg-[#0d121b]">{bank.name} {bank.country ? `(${bank.country})` : ''}</option>
                                        ))}
                                    </select>
                                    <span className="material-symbols-outlined absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-text-tertiary">account_balance</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                            <div>
                                <label className="text-[10px] font-black text-text-tertiary uppercase tracking-widest mb-3 block">Account Number / IBAN</label>
                                <input type="text" placeholder={transferRegion === 'Africa' ? "0123456789" : "GB29 HBUK 1234 5678"} className="w-full h-14 bg-white/5 border border-white/10 rounded-xl px-4 focus:outline-none focus:border-primary text-sm font-bold" />
                            </div>
                            <div>
                                <label className="text-[10px] font-black text-text-tertiary uppercase tracking-widest mb-3 block">Recipient Full Name</label>
                                <input type="text" placeholder="John Doe" className="w-full h-14 bg-white/5 border border-white/10 rounded-xl px-4 focus:outline-none focus:border-primary text-sm font-bold" />
                            </div>
                        </div>

                        <div className="mb-8">
                            <label className="text-[10px] font-black text-text-tertiary uppercase tracking-widest mb-3 block">Amount ({transferRegion === 'Africa' ? 'NGN' : 'EUR'})</label>
                            <div className="relative">
                                <span className="absolute left-6 top-1/2 -translate-y-1/2 text-2xl font-black text-text-tertiary">{transferRegion === 'Africa' ? '₦' : '€'}</span>
