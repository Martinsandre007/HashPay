...
const SendScreen: FC<SendScreenProps> = ({ onBack, onConfirm, onNavigate }) => {
  const { wallets, sendFunds, isLoading: isWalletLoading, contacts, afriexTransfer, spendingLimit, isLockedDown } = useWallet();
  ...
  const handleManualConfirm = async () => {
    if (isLockedDown) {
      showToast('Wallet is in EMERGENCY LOCKDOWN. Transfer disabled.', 'error');
      return;
    }
    ...
    if (val > spendingLimit.daily) {
      showToast(`Amount exceeds daily spending limit of ₦${spendingLimit.daily.toLocaleString()}`, 'error');
      return;
    }

    const confirmMsg = transferType === 'sui'
      ? `Authorize On-Chain Transfer\nAmount: ${amount} ${selectedAsset}\nTo: ${recipient}\n\nProceed?`
      : `Confirm Cross-Border Transfer\nAmount: ${amount} ${targetCurrency}\nTo: ${recipientName || recipient}\n\nProceed?`;
  ...
  return (
    <div className="bg-background-dark text-white font-display min-h-screen flex flex-col overflow-x-hidden animate-fade-in pb-32 max-w-7xl mx-auto w-full">
      <header className="sticky top-0 z-50 flex items-center justify-between px-6 py-4 glass border-b border-white/5 lg:hidden">
      ...
      <main className="flex-1 flex flex-col p-6 gap-8">
        ...
        <section className="flex flex-col gap-3">
          ...
          <div className="relative group">
            <input
              className="w-full h-16 pl-6 pr-14 rounded-2xl bg-surface-dark/50 border border-white/10 focus:border-primary focus:ring-4 focus:ring-primary/10 text-lg font-bold text-white placeholder:text-gray-600 outline-none transition-all shadow-xl"
          ...
          {transferType === 'afriex' && (
            <input
              className="w-full h-14 px-6 rounded-2xl bg-surface-dark/30 border border-white/5 focus:border-[#22c55e] text-sm font-bold text-white placeholder:text-gray-700 outline-none transition-all"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          <div className="flex flex-col gap-8">
            <section>
              ...
              <div className="flex overflow-x-auto lg:grid lg:grid-cols-4 lg:overflow-x-visible gap-5 hide-scrollbar pb-2 snap-x">
                <button
                  onClick={() => onNavigate(AppScreen.CONTACTS)}
          ...
          <div className="flex flex-col gap-10">
            <section className="flex flex-col items-center gap-6 mt-4 lg:mt-0">
            ...
            <section className="bg-surface-dark/30 rounded-[2.5rem] p-8 border border-white/5 shadow-2xl relative overflow-hidden group">
      ...
      <div className="fixed lg:relative bottom-0 left-0 w-full glass lg:bg-transparent lg:border-none p-6 pb-10 lg:pb-6 z-40 lg:mt-auto">
