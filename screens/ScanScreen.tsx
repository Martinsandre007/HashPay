...
const ScanScreen: React.FC<ScanScreenProps> = ({ onBack, onNavigate }) => {
      ...
      if (videoRef.current && videoRef.current.readyState === videoRef.current.HAVE_ENOUGH_DATA) {
        ...
        if (canvas) {
          ...
          if (context) {
            ...
            if (code) {
              setIsScanning(false);
              showToast(`QR Code Detected: ${code.data.slice(0, 8)}...`, 'success');
              ...
              setTimeout(() => {
                onNavigate(AppScreen.SEND, { address: code.data });
              }, 1000);
  ...
  return (
    <div className="bg-black text-white font-display min-h-screen flex flex-col overflow-hidden animate-fade-in relative">
      {/* Real Camera Preview */}
      <video
        ref={videoRef}
      ...
      <header className="absolute top-0 inset-x-0 z-50 flex items-center justify-between px-6 py-8 bg-gradient-to-b from-black/80 to-transparent">
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="flex items-center gap-1.5 mt-1">
            <span className={`size-1.5 rounded-full animate-pulse ${isScanning ? 'bg-green-500' : 'bg-primary'}`}></span>
            <span className={`text-[10px] font-bold uppercase tracking-widest ${isScanning ? 'text-green-500' : 'text-primary'}`}>
              {isScanning ? 'Camera Ready' : 'Processing...'}
            </span>
          </div>
        </div>
        <button
      ...
      {/* Footer Branding */}
      <footer className="p-10 flex justify-center opacity-30 z-10">
