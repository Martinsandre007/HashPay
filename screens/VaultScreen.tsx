...
const VaultScreen: React.FC<VaultScreenProps> = ({ onBack }) => {
  ...
  const toggleFeature = (id: string) => {
    if (id === 'panic') {
      const newValue = !isPanicMode;
      setIsPanicMode(newValue);
      showToast(newValue ? 'PANIC MODE ACTIVATED' : 'Panic Mode Deactivated', newValue ? 'error' : 'success');
    } else if (id === 'time') {
      const newValue = !isTimeLocked;
      setIsTimeLocked(newValue);
      showToast(newValue ? '24h Time Lock Enabled' : 'Time Lock Disabled', 'info');
    } else {
      showToast('Feature managed by system policy', 'info');
    }
  };
      ...
      <main className="flex-1 p-6 flex flex-col gap-8 pt-8">
        ...
        {activeTab === 'storage' ? (
          <div className="flex flex-col gap-6 animate-slide-up">
            {isPanicMode ? (
              <div className="flex flex-col items-center justify-center py-20 opacity-50">
                <span className="material-symbols-outlined text-6xl text-red-500 mb-4">lock_clock</span>
                <h3 className="text-xl font-black uppercase tracking-widest text-red-500">Vault Lockdown</h3>
                <p className="text-[10px] font-bold text-gray-500 mt-2">Assets hidden due to active Panic Mode.</p>
              </div>
            ) : (
              <>
          ...
          <div className="flex flex-col gap-6 animate-slide-up">
            {/* Security Features */}
            {[
              { id: 'bio', title: 'Biometric Unlock', desc: 'Secure your vault with FaceID or Fingerprint', icon: 'fingerprint', danger: false },
              { id: 'time', title: 'Time Lock', desc: 'Assets require 24h wait for large releases', icon: 'timer', danger: false },
              { id: 'panic', title: 'Panic Mode', desc: 'Immediately hide and lock all vault assets', icon: 'gpp_maybe', danger: true }
            ].map((feat, i) => (
              <div key={i} className="flex items-center justify-between p-6 bg-surface-dark/40 border border-white/5 rounded-[2rem] hover:bg-surface-dark transition-all group">
                ...
                <div
                  ...
                  className={`size-10 rounded-xl flex items-center justify-center transition-all cursor-pointer hover:scale-110 active:scale-95 ${(feat.id === 'panic' && isPanicMode) || (feat.id === 'time' && isTimeLocked) || (feat.id === 'bio' && true) // mocking bio always on for now
                    ? 'bg-green-500/20 text-green-500' : 'bg-white/5 text-gray-700'
                    }`}
                >
                  <span className="material-symbols-outlined text-xl">
                    {(feat.id === 'panic' && isPanicMode) || (feat.id === 'time' && isTimeLocked) || (feat.id === 'bio' && true) ? 'toggle_on' : 'toggle_off'}
                  </span>
                </div>
              </div>
            ))}
